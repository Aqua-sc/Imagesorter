<script>
    ...
    function download() {
        if (name) {
            let zip = new JSZip();
            files.forEach(a => zip.file(`${name}/${a.categoryValue.name}/${a.file.name}`, a.file));
...
<div class="flex gap-1 mt-1 place-items-center">
    ...
    <input class="p-1 border-blue-500 bg-blue-300 bg-opacity-95 border-2 rounded-md text-white font-bold placeholder-gray-200 text-sm focus:outline-0 focus:ring-2" bind:value={name} placeholder="Folder name here"> 
</div>
